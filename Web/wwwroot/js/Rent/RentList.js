        ...
        axios.get('/api/houses').then(({
                ...
                let card_list = filterList.reduce((accumulator, current) => {
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <input type="hidden" name="houseId" value="${current.houseId}">
                            <h2 class="card-title secondary-color mb-2">${currency(current.price).format()}</h2>
                            <p class="card-subtitle mb-2 d-flex">
                                <span><i class="fas fa-bed"></i> ${current.bedrooms} Bd</span> 
                                <span class="mx-1 mx-md-2"><i class="fas fa-toilet-paper"></i> ${current.bathrooms} Ba </span>
                                <span><i class="fas fa-ruler-combined"></i> ${current.size} sqft</span>
                            </p>
                            <p class="card-text"> <span><i class="fas fa-search-location"></i> ${current.province}</span> </p>
                                <div class="d-flex justify-content-end align-items-center">
                                    <div>
                                        <a href="/Rent/House/${current.houseId}" class="btn btn-danger"><i
                                            class="fas fa-truck-moving"></i> Rent</a>
                                        <a href="/House/Details/${current.houseId}" class="btn btn-outline-info"><i
                ...
                Array.from(document.getElementsByClassName('card')).forEach(card => {
                    card.addEventListener('click', event => {
                        ...
                        axios.get(`/api/houses/${inputId.value}`).then(({
                            data
                        }) => {
                            document.getElementById('home-details-example').classList.remove('d-flex');
                            ...
                            document.getElementById('home-price').innerHTML = currency(data.price).format();
